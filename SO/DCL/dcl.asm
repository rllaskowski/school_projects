...
;               bl - oczekiwana ilosc wystapien znaku przed rozpoczeciem procedury
;               bh - oczekiwana dlugosc parametru
; wyjscie:      kazdy kod znaku w tablicy z permutacja zostaje zmniejszony o kod '1'
_check_param:
...
; wyjscie       rax - przepermutowany kod znaku 
_parse_sign:
    inc         bl                      ; zwieksz pozycje bębenka r
; sprawdzamy czy bebenek l jest na ktorejs z pozycji obrotowych
...
_start:
    ...
    pop         r10                     ; zdejmij do r10 adres klucza
    checkParam  r10, 0, 2               ; sprawdz poprawnosc klucza
    mov         bx, [r10]               ; wstaw do bx klucz - w bh poczatkowa pozycja l - w bl poczatkowa pozycja r
_input_loop:
    ...
    test        eax, eax                ; jesli wczytano 0 ilosc znakow
    jz          _input_end              ; skoncz wczytywanie

    mov         edx, eax                ; w rdx trzymamy ilosc znakow do wypisania = ilosci wczytanch znakow
...
_check_loop:
    ...
    cmp         cx, dx                  ; czy zostaly przejrzane wszystkie znaki? na gorze stosu jest liczba wczytanych znakow
